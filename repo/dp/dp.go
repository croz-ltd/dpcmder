...
func (r *DpRepo) GetFileTypeFromPath(m *model.Model, filePath string) model.ItemType {
...
func (r *DpRepo) GetFileType(m *model.Model, parentPath, fileName string) model.ItemType {
	...
	if config.DpUseRest() {
		...
		if len(filestore) == 1 {
			return model.ItemDirectory
		} else if len(file) == 1 {
			return model.ItemFile
		} else if len(file) == 0 {
			return '0'
		}
...
func (r *DpRepo) UpdateFile(m *model.Model, parentPath, fileName string, newFileContent []byte) bool {
	...
	if config.DpUseRest() {
		...
		restMethod := "POST"
		if fileType == model.ItemFile {
			updateFilePath = r.GetFilePath(parentPath, fileName)
			restMethod = "PUT"
		} else if fileType == model.ItemDirectory {
		...
		if fileType == model.ItemDirectory {
			logging.LogFatal(fmt.Sprintf("repo.dp.UpdateFile() - can't upload file '%s' to '%s', directory with same name exists.", fileName, parentPath))
		} else {
			filePath := r.GetFilePath(parentPath, fileName)
			somaRequest := "<soapenv:Envelope xmlns:soapenv=\"http://schemas.xmlsoap.org/soap/envelope/\"><soapenv:Body>" +
